    ...
    <aside className="background-light900_dark200 light-border sticky right-0 top-20 flex h-[calc(100vh-5rem)] flex-col justify-between border-l p-5 shadow dark:shadow-none max-xl:hidden max-sm:hidden lg:w-[350px]">
      <div>
        ...
        <div className="mt-5 space-y-7">
          {topQuestions.map((question: { id: Key | null | undefined; title: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined; }) => (
            <Link
              key={question.id}
      ...
      <div>
        ...
        <div className="mt-5 space-y-3">
          {popularTags.map((tag: { id: Key | null | undefined; name: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined; numberOfQuestions: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined; }) => (
            <div key={tag.id} className="flex items-center justify-between">
              ...
              <p className="text-xs font-medium text-gray-600">{500}</p>
            </div>
          ))}
        </div>
      </div>
